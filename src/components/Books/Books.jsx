      ...
      <ul className='book_list d-flex flex-wrap justify-content-between'>
        {usersArr.map((book)=>{
          if(book.Id === loggedUser.Id){
            return <li key={loggedUser.Id} className='book_list_item d-flex align-items-center justify-content-between'><div className='span_img' style={{backgroundImage: `url(${book.Picture})`}}></div>My Book</li>
          } else {
